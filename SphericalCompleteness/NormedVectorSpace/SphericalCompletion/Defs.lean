...
/--
`SphericalCompletion.immediateExtensionSubmodules E E₀ f` is the set of `𝕜`-submodules `M ≤ E₀`
such that:
...
-/
def SphericalCompletion.immediateExtensionSubmodules {𝕜 : Type*} [NontriviallyNormedField 𝕜]
...
/-- Clean membership criterion for `SphericalCompletion.immediateExtensionSubmodules`, expressed
entirely in the ambient
space `E₀`: `M` contains `f.range`, and any `v ∈ M` metrically orthogonal to `f.range`
(in `E₀`) is `0`. -/
lemma SphericalCompletion.mem_immediateExtensionSubmodules_iff
    {𝕜 : Type*} [NontriviallyNormedField 𝕜]
    {E : Type*} [NormedAddCommGroup E] [NormedSpace 𝕜 E] [IsUltrametricDist E]
    {E₀ : Type*} [NormedAddCommGroup E₀] [NormedSpace 𝕜 E₀] [IsUltrametricDist E₀]
    [SphericallyCompleteSpace E₀] {f : E →ₗᵢ[𝕜] E₀} {M : Submodule 𝕜 E₀} :
    M ∈ SphericalCompletion.immediateExtensionSubmodules E E₀ f ↔
      ∃ _ : f.range ≤ M,
        ∀ v : M, Metric.infDist (v : E₀) f.range = ‖(v : E₀)‖ → v = 0 := by
  simp only [SphericalCompletion.immediateExtensionSubmodules, Set.mem_setOf_eq, IsImmediate]
...
-/
lemma SphericalCompletion.immediateExtensionSubmodules_nonempty
    {𝕜 : Type*} [NontriviallyNormedField 𝕜]
    (E : Type*) [NormedAddCommGroup E] [NormedSpace 𝕜 E] [IsUltrametricDist E]
    (E₀ : Type*) [NormedAddCommGroup E₀] [NormedSpace 𝕜 E₀] [IsUltrametricDist E₀]
    [SphericallyCompleteSpace E₀]
    (f : E →ₗᵢ[𝕜] E₀)
    : (SphericalCompletion.immediateExtensionSubmodules E E₀ f).Nonempty := by
  refine ⟨f.range, SphericalCompletion.mem_immediateExtensionSubmodules_iff.2
    ⟨le_rfl, fun v hv ↦ ?_⟩⟩
...
consider the set `SphericalCompletion.immediateExtensionSubmodules E E₀ f` of submodules `M ≤ E₀`
that contain the
...
-/
theorem SphericalCompletion.exists_maximal_immediateExtensionSubmodule
    (𝕜 : Type*) [NontriviallyNormedField 𝕜]
    (E : Type*) [NormedAddCommGroup E] [NormedSpace 𝕜 E] [IsUltrametricDist E]
    (E₀ : Type*) [NormedAddCommGroup E₀] [NormedSpace 𝕜 E₀] [IsUltrametricDist E₀]
    [SphericallyCompleteSpace E₀]
    (f : E →ₗᵢ[𝕜] E₀) :
    ∃ m, Maximal (fun x ↦ x ∈ SphericalCompletion.immediateExtensionSubmodules E E₀ f) m := by
  apply zorn_le₀
  intro C hC1 hC2
  if hC : ¬ C.Nonempty then
    refine ⟨(SphericalCompletion.immediateExtensionSubmodules_nonempty E E₀ f).some,
      Set.Nonempty.some_mem (SphericalCompletion.immediateExtensionSubmodules_nonempty E E₀ f), ?_⟩
    intro c hc
    contrapose hC
    use c
  else
  ...
  refine ⟨SphericalCompletion.mem_immediateExtensionSubmodules_iff.2
  ...
  obtain ⟨hc, himm⟩ := SphericalCompletion.mem_immediateExtensionSubmodules_iff.1 (hC1 N.2)
  apply Subtype.ext
...
-/
noncomputable def SphericalCompletion.embedding (𝕜 : Type*) [NontriviallyNormedField 𝕜]
