...
(in `E₀`) is `0`. -/
lemma mem_imm_ext_iff {𝕜 : Type*} [NontriviallyNormedField 𝕜]
    {E : Type*} [NormedAddCommGroup E] [NormedSpace 𝕜 E] [IsUltrametricDist E]
    {E₀ : Type*} [NormedAddCommGroup E₀] [NormedSpace 𝕜 E₀] [IsUltrametricDist E₀]
    [SphericallyCompleteSpace E₀] {f : E →ₗᵢ[𝕜] E₀} {M : Submodule 𝕜 E₀} :
    M ∈ imm_ext_in_sph_comp E E₀ f ↔
      ∃ _ : f.range ≤ M,
        ∀ v : M, Metric.infDist (v : E₀) f.range = ‖(v : E₀)‖ → v = 0 := by
  simp only [imm_ext_in_sph_comp, Set.mem_setOf_eq, IsImmediate]
...
: (imm_ext_in_sph_comp E E₀ f).Nonempty := by
  refine ⟨f.range, mem_imm_ext_iff.2 ⟨le_rfl, fun v hv => ?_⟩⟩
...
(f : E →ₗᵢ[𝕜] E₀) : ∃ m, Maximal (fun x ↦ x ∈ imm_ext_in_sph_comp E E₀ f) m := by
  ...
  refine ⟨mem_imm_ext_iff.2 ⟨hf_le, fun x horth => ?_⟩, fun M hM z hz =>
  ...
  obtain ⟨hc, himm⟩ := mem_imm_ext_iff.1 (hC1 N.2)
  apply Subtype.ext
