...
∃ F' : Submodule 𝕜 E, IsCompl F F' ∧ (F ⟂ₛ F') := by
  ...
  use LinearMap.ker T
  refine ⟨orth_of_orthcomp 𝕜 F T hT1, ?_⟩
  rw [sorth_symm]
  unfold SOrth MOrth
  intro x hx
  ...
  · apply (le_infDist (Submodule.nonempty F)).2
    ...
    have : ‖y‖ ≤ ‖x - y‖ := by
      have : T (x - y) = -y := by
        simp only [map_sub, hx, hT1 y hy, zero_sub, NegMemClass.coe_neg]
      rw [← norm_neg, ← this]
...
[NormedSpace 𝕜 E]
(F : Submodule 𝕜 E) [SphericallyCompleteSpace F]
: Submodule 𝕜 E := (exists_orthcomp_of_spherically_complete_space 𝕜 F).choose

theorem orthcomp_isCompl (𝕜 : Type*) [NontriviallyNormedField 𝕜]
{E : Type*} [NormedAddCommGroup E] [iud : IsUltrametricDist E]
[NormedSpace 𝕜 E]
(F : Submodule 𝕜 E) [SphericallyCompleteSpace F] :
IsCompl F (OrthComp 𝕜 F) := (exists_orthcomp_of_spherically_complete_space 𝕜 F).choose_spec.1

theorem orthcomp_sorth (𝕜 : Type*) [NontriviallyNormedField 𝕜]
{E : Type*} [NormedAddCommGroup E] [iud : IsUltrametricDist E]
[NormedSpace 𝕜 E]
(F : Submodule 𝕜 E) [SphericallyCompleteSpace F] :
(F ⟂ₛ (OrthComp 𝕜 F)) :=
(exists_orthcomp_of_spherically_complete_space 𝕜 F).choose_spec.2
