...
* Orthogonality of vectors is symmetric (`orth_symm`) and admits two reformulations: it coincides
  with **Birkhoff–James orthogonality** (`orth_iff_birkhoff_james_orth`) and with the ultrametric
...
* All three relations are invariant under nonzero scalar multiplication
  (`smul_orth_iff_orth_of_nonzero`, `orth_smul_iff_orth_of_nonzero`,
  `smul_morth_iff_morth_of_nonzero`).
* Metric orthogonality to a subspace is orthogonality to each of its vectors
  (`morth_iff_forall_orth`); subspace orthogonality unfolds to orthogonality of all pairs of
  vectors (`sorth_iff_forall_orth`) and is symmetric (`sorth_symm`).
* A vector that both lies in `F` and is orthogonal to `F` is zero (`eq_zero_of_morth_of_mem`).
-/
...
technical direction from which the symmetry equivalence `orth_symm` is assembled; the argument
...
for every scalar `a`. This is the counterpart of `smul_orth_of_orth` obtained through symmetry. -/
...
direction underlying the symmetry equivalence `sorth_symm`, obtained by swapping the roles of the
